...
func TestGovernanceMetricsAggregateInboxAndFailureEvidence(t *testing.T) {
	...
	_, err := db.Exec(`INSERT INTO notification_event_failures (id, workspace_id, event_id, event_type, source, source_event_id, stage, error_code, attempt, disposition, retryable, next_attempt_at, fencing_token, occurred_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
