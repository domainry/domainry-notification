...
// Validator owns portable template validation over module-owned domain values.
// SDK wire values are converted by adapters before entering this boundary.
type Validator struct{ capabilities *Capabilities }
...
func (v *Validator) ValidateAll(values []templatemodel.Template) error {
	...
	for index := range values {
		if err := v.Validate(values[index]); err != nil {
			return fmt.Errorf("notification_templates[%d]: %w", index, err)
		}
		for _, fallback := range values[index].Fallbacks {
			if !seen[fallback.TemplateKey] {
				return invalid("backend.notification.fallback_template_not_found", "template_key", values[index].Key, "fallback_template_key", fallback.TemplateKey)
			}
		}
	}
	return nil
...
func (v *Validator) Validate(value templatemodel.Template) error {
	...
	entry, known := v.capabilities.entries[capabilityKey(channel, provider)]
	if !known {
	...
	for _, locale := range locales {
		if err := validateContent(value, locale, value.Locales[locale], variables, entry); err != nil {
			return err
		}
	}
	return nil
...
func validateVariables(value templatemodel.Template) (map[string]templatemodel.Variable, error) {
	variables := make(map[string]templatemodel.Variable, len(value.Variables))
...
func validateFallbacks(value templatemodel.Template) error {
...
func validateContent(value templatemodel.Template, locale string, content templatemodel.Content, variables map[string]templatemodel.Variable, entry capabilityEntry) error {
	...
	if strings.TrimSpace(content.HTML) != "" && !capability.SupportsHTML || strings.TrimSpace(content.Markdown) != "" && !capability.SupportsMarkdown {
		return invalid("backend.notification.template_channel_mismatch", "template_key", value.Key, "locale", locale)
	}
	...
	if content.ProviderTemplate != nil {
		...
		if entry.validate != nil && entry.validate(content.ProviderTemplate) != nil {
...
func validateTokens(templateKey, locale, source string, variables map[string]templatemodel.Variable) error {
