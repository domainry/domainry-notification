...
func TestCatalogIsImmutableAndDetectsActionContractConflicts(t *testing.T) {
	...
	first.Actions = []inbox.ActionDescriptor{{Key: "workflow.task.open", Kind: "route", ResourceType: "workflow_task", SurfaceRoutes: map[string]string{"business_workspace": "workflow.task.detail"}}}
	...
	first.Actions[0].SurfaceRoutes["business_workspace"] = "mutated"
	descriptor, found := catalog.Action("workflow.task.open")
	if !found || descriptor.SurfaceRoutes["business_workspace"] != "workflow.task.detail" {
		t.Fatalf("descriptor=%+v found=%v", descriptor, found)
	}
	descriptor.SurfaceRoutes["business_workspace"] = "also.mutated"
	descriptor, _ = catalog.Action("workflow.task.open")
	if descriptor.SurfaceRoutes["business_workspace"] != "workflow.task.detail" {
	...
	second.Actions = []inbox.ActionDescriptor{{Key: "workflow.task.open", Kind: "route", ResourceType: "automation_rule", SurfaceRoutes: map[string]string{"business_workspace": "automation.rule.detail"}}}
