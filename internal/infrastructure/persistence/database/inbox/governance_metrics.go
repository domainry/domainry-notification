...
func (s *Store) eventFailureMetrics(ctx context.Context, workspaceID notification.WorkspaceID, since string) (inbox.FailureMetrics, error) {
	...
	statement, args, err := builder.NewWorkspaceSelectBuilder(s.Renderer, "_notification_event_failures", workspaceID.String()).Projections(projections...).Where(predicate).Build()
	if err != nil {
		return result, err
	}
	...
	for _, dimension := range dimensions {
		statement, args, err := builder.NewWorkspaceSelectBuilder(s.Renderer, "_notification_event_failures", workspaceID.String()).Projections(builder.Project(builder.Column(dimension.column)), builder.Project(builder.CountAll())).Where(predicate).GroupBy(builder.Column(dimension.column)).OrderBy(builder.DescendingExpression(builder.CountAll()), builder.Ascending(dimension.column)).Build()
		if err != nil {
			return result, err
		}
...
func (s *Store) inboxAggregateSummary(ctx context.Context, workspaceID string, predicate builder.Predicate) (inbox.Aggregate, error) {
	value := inbox.Aggregate{}
	statement, args, err := builder.NewWorkspaceSelectBuilder(s.Renderer, "_notification_inbox_items", workspaceID).Projections(inboxAggregateProjections()...).Where(predicate).Build()
	if err != nil {
		return value, err
	}
...
func (s *Store) inboxAggregateRows(ctx context.Context, workspaceID string, predicate builder.Predicate, column string) ([]inbox.Aggregate, error) {
	...
	statement, args, err := builder.NewWorkspaceSelectBuilder(s.Renderer, "_notification_inbox_items", workspaceID).Projections(projections...).Where(predicate).GroupBy(builder.Column(column)).OrderBy(builder.DescendingExpression(builder.CountAll()), builder.Ascending(column)).Build()
	if err != nil {
		return nil, err
	}
	rows, err := s.Database.QueryContext(ctx, statement, args...)
