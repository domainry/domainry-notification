...
func TestRemoteFactoryPassesDeploymentNeutralContractSuite(t *testing.T) {
	...
	contracttest.Run(t, func(testing.TB) (notificationsdk.Factory, notificationsdk.ApplicationRef) {
		return notificationremote.NewFactory(notificationremote.Config{BaseURL: httpServer.URL, ServiceCredential: "service-token", HTTPClient: httpServer.Client()}), notificationsdk.ApplicationRef{WorkspaceID: "workspace", ApplicationKey: "runtime"}
	})
}
